...
export default function EditProjectPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        ...
        const fetchProjectData = async () => {
            try {
                const existingProject = await ProjectsAPI.getProjectById(Number(projectId));
                ...
                // Define a URL da imagem existente para a pré-visualização
                setPreviewUrl(existingProject.imageUrl);

            } catch (err) {
                console.error("Erro ao buscar dados do projeto:", err);
                setError("Não foi possível carregar os dados do projeto para edição.");
            } finally {
    ...
    // Limite: 5MB
    const MAX_FILE_SIZE = 3 * 1024 * 1024; // 5MB em bytes
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            const response = await ProjectsAPI.updateProject(Number(projectId), payload);
            ...
            if (error.response) {
                ...
                switch (status) {
                    case 403:
                        setError('Acesso negado. Você não tem permissão para editar projetos ou sua sessão expirou. Faça login novamente.');
                        setTimeout(() => {
                            logout();
                            router.push('/login');
                        }, 3000);
                        break;
                    case 413:
                        // Adicionamos uma verificação para garantir que imageFile não é nulo
                        if (imageFile) {
                            setError(`Arquivo muito grande para o servidor. Tamanho: ${formatFileSize(imageFile.size)}. Tente uma imagem menor.`);
                        } else {
                            // Fallback caso imageFile seja nulo por algum motivo inesperado
            ...
            <main className="flex-1 overflow-y-auto overflow-x-hidden px-4 py-4">
                                    ...
                                    <div className={`${marope.className} flex flex-col justify-between pr-8`}>
                                        <div className="space-y-6">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-500 mt-1">
                                                    Tamanho máximo: {MAX_FILE_SIZE / (1024 * 1024)}MB • Formatos: PNG,
                                                    JPEG, WebP
                                                    <br/>
                                        ...
                                        <div className="flex items-center justify-center gap-4 mt-8">
                                            <button
                                                type="submit"
                                                disabled={loading}
                                                className={`${marope.className} cursor-pointer w-32 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-lg py-1.5 rounded-md transition-colors disabled:bg-gray-500 disabled:cursor-not-allowed`}
                                            >
                ...
                {showSuccessModal && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div
                            className="bg-[#1E1E1E] text-white p-6 rounded-xl shadow-lg max-w-md w-full text-center mx-4">
                            ...
                            <p className={`${marope.className} mb-6 text-gray-300`}>
                                Projeto editado com sucesso! Você será redirecionado para o dashboard.
                            </p>
                            <button
