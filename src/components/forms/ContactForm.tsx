            ...
            <div className={`bg-[#1a1a1a] p-6 md:p-8 rounded-2xl shadow-lg ${marope.className} w-full max-w-xl`}>
                ...
                <form onSubmit={handleSubmit} className="space-y-5 w-full h-auto">
                    <div className="w-auto h-auto mr-9">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-16 gap-y-4">
                            <div>
                                <label htmlFor="firstName"
                                       className="block text-sm font-medium text-gray-300 mb-2 ">Nome*</label>
                                <input type="text" id="firstName" value={formData.firstName} onChange={handleChange}
                                       className={inputClassName} required placeholder="Guilherme"/>
                            </div>
                            <div className="pb-2">
                                <label htmlFor="lastName"
                                       className="block text-sm font-medium text-gray-300 mb-2 ">Sobrenome*</label>
                                <input type="text" id="lastName" value={formData.lastName} onChange={handleChange}
                                       className={inputClassName} required placeholder="Araújo"/>
                            </div>
                        </div>

                        <div className="pb-2">
                            <label htmlFor="phone"
                                   className="block text-sm font-medium text-gray-300 mb-2 ">Telefone</label>
                            <IMaskInput
                                mask="(00) 00000-0000"
                                type="tel"
                                id="phone"
                        ...
                        <div className="pb-2">
                            <label htmlFor="email"
                                   className="block text-sm font-medium text-gray-300 mb-2 ">E-mail*</label>
                            <input type="email" id="email" value={formData.email} onChange={handleChange}
                                   className={inputClassName} required placeholder="seu-email@exemplo.com"/>
                        </div>

                        <div className="pb-2">
                            <label htmlFor="message"
                                   className="block text-sm font-medium text-gray-300 mb-2 ">Mensagem*</label>
                            <textarea id="message" rows={5} value={formData.message} onChange={handleChange}
                                      className={`${inputClassName} resize-none`} required
                                      placeholder="Dúvidas, Propostas, Sugestões, Elogios..."/>
                        </div>
